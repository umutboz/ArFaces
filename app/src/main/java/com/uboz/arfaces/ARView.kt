...
import com.uboz.arfaces.utilities.SnackbarHelper
import com.uboz.arfaces.utilities.TapHelper

/** Contains UI elements for Hello AR. */
class ArView(val activity: ARActivity, r_activity_layout : Int,
             r_surface_view: Int, r_settings_button: Int, r_settings_menu : Int ) : DefaultLifecycleObserver {
    val root = View.inflate(activity, r_activity_layout, null)
    val surfaceView = root.findViewById<GLSurfaceView>(r_surface_view)
    val settingsButton =
        root.findViewById<ImageButton>(r_settings_button).apply {
            setOnClickListener { v ->
                PopupMenu(activity, v).apply {
                    setOnMenuItemClickListener { item ->
                        when (item.itemId) {
                            1 /*R.id.depth_settings */ -> {
                                //launchDepthSettingsMenuDialog()
                            }
                            2 /*R.id.plane_finding_settings */ -> {
                                //launchPlaneFindingSettingsMenuDialog()
                            }
                            else -> null
                        } != null
                    }
                    inflate(r_settings_menu)
                    show()
                }
            }
        }
